...
- Cached binary is re-verified against the release's `SHA256SUMS` periodically (default every 600 s; override with `EGGS_VERIFY_INTERVAL` in seconds, or `EGGS_SKIP_VERIFY=1` to always trust the cache for offline / CI use). Mismatch triggers a re-download; an inconclusive check (no network, no sha256 tool) falls back to the cache without bumping the verify marker.
